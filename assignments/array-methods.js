...
let allCaps = [];
runners.map(function(first) {
  allCaps.push(first.first_name.toUpperCase());
});
console.log(allCaps);
...
let largeShirts = [];
runners.filter(function(word) {
  if(word.shirt_size === "L") {
    largeShirts.push(word.first_name, word.last_name);
  }
});
